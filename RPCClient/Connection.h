...
public:
	...
	explicit Connection(struct xio_connection* conn): m_conn{ conn }, m_conn_flag{ false }, m_blk_num{ 0 }
	{
		// register the buffer
	...
	// copy constructor
	Connection(const Connection& conn): m_conn{conn.m_conn}, m_conn_flag{ conn.m_conn_flag }, m_header_buffer{ new char[BUFFER_SIZE] }, m_data_buffer{ new char[BUFFER_SIZE] }, m_blk_num{ conn.m_blk_num }
	{
	...
	// moving constructor
	Connection(Connection&& conn): m_conn{ conn.m_conn }, m_conn_flag{ conn.m_conn_flag }, m_header_buffer{ conn.m_header_buffer }, m_data_buffer{ conn.m_data_buffer }, m_blk_num{ conn.m_blk_num }
	{
	...
	Connection& operator=(Connection&& conn)
	{
		...
		std::swap(m_channel, conn.m_channel);
		std::swap(m_blk_num, conn.m_blk_num);
	...
	void process_request(struct xio_msg* req, const int sn);

	void process_response(struct xio_msg* rsp, const int sn);

	// can be extend to send/recv interfaces
	void fire_communication();
...
private:
	...
	int m_blk_num;									// number of the data block
